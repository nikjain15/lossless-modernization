...
It installs into your repository, reads your actual code, and writes real artifacts into a `docs/modernization/` folder. It keeps a `STATE.md` that carries the program's memory across months and team changes. It is not a chatbot that answers a question and forgets you.

Version 0.1.0, beta. The criteria come from twelve years of doing this work, and the toolkit itself is new: corrections are welcome and wanted.
...
| **readiness** | Before committing to a date | `readiness.md`: twelve categories scored red, amber, green, with an owned action for every red |
| **plan-review** | You have a plan and want it stress-tested | `plan-review.md`: which documented failure your plan resembles, and the line in your plan that creates the resemblance |
| **cutover** | You are approaching go-live | `cutover-runbook.md` and `rollback-plan.md`, behind a six-condition gate |

...
each real switch. Not once at the start of the program: before each one. This was affordable
because migration moved populations incrementally, so a rollback was a routing change on one
account and fund group rather than a restore of a system. A big bang cannot rehearse its rollback
at realistic scale, which is why big-bang rollback plans stay hypothetical.

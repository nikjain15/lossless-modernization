...
Twelve categories, scored 0 to 5. Each category says what can be read from code and what has to be asked, because the two are not interchangeable.
...
| 1 | Someone has raised it; nothing exists |
| 2 | A plan exists on paper, unowned or unfunded |
| 3 | Owned and funded, not yet proven to work |
| 4 | Working, with evidence, some gaps remain |
| 5 | Working, evidenced, and would survive an audit |

**Red is 0 to 2. Amber is 3. Green is 4 to 5.** Any category at 0 or 1 in the correctness group (03, 04) or the cutover group (05) should block a committed go-live date, regardless of the total.

Report the profile, never a single average. A program with ten fives and two zeroes is not "83 percent ready"; it is one bad weekend away from being a case study.
...
**5 looks like:** current behavior is captured in a form a non-engineer can review, and it has been checked against the running system.
...
**5 looks like:** knowledge holders are named, their time is committed in writing, and capture is underway rather than planned.
...
**5 looks like:** semantics are documented per critical field, differences are agreed and signed, and a comparison mechanism exists.
...
**5 looks like:** a comparison runs on real inputs, produces a reviewable report, and three named parties sign it.
...
**5 looks like:** migration is incremental, rollback is rehearsed, abort criteria are written and a named person owns the call.
...
**5 looks like:** an explicit not-doing list, and value in production within a quarter.
...
**5 looks like:** every choice traces to a named constraint, and the simplest option was considered and rejected on record.
...
**5 looks like:** all four are costed as line items, with a stated contingency and a named owner for the number.
...
**5 looks like:** one named accountable owner, decision rights written down, and a stop authority independent of the delivery incentive.
...
**5 looks like:** internal ownership of the domain model and the evidence, with exit terms agreed.
...
**5 looks like:** AI accelerates comprehension, humans ratify intent, and execution-based comparison verifies anything replicated.
...
**5 looks like:** the status quo is quantified as an option with a price, and it appears in the business case.

---

## Mapping a low score to an action

For every category scored 0 to 2, the report must state the smallest next action that would move it to 3. Not a chapter to read: an action with an owner. A scorecard that produces reading is a diagnostic. A scorecard that produces owned actions is a tool.

---
name: review-plan
description: Review a legacy modernization or migration plan against twelve sourced failure categories and eight documented public failures, and name which historical failure the plan most resembles. Use when the user has a modernization plan, migration approach, target architecture, cutover plan, or business case and wants it stress-tested, critiqued, risk-assessed, or checked before committing to it. Also use when asked what could go wrong with a modernization, or whether a plan is sound.
---
...
2. `modernization/` in the repo, especially `STATE.md`, any wave plan, business case, or cutover runbook
...
Write to `modernization/plan-review.md`, creating the folder if needed. Then update `modernization/STATE.md`: add the review date, the top three risks, and any open questions the user must answer. If `STATE.md` does not exist, create it with the program name, current phase, and the risks you found.
...
who signs correctness, whether budget covers evidence work, who can stop the program.>
```
